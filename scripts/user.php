...
if(!in_array($action,['create','reset','disable'])||!filter_var($email,FILTER_VALIDATE_EMAIL)){fwrite(STDERR,"Usage: php scripts/user.php create|reset|disable email [name]\nFor create/reset enter a password on the next line (12+ characters).\n");exit(1);}
if($action==='disable'){db()->prepare('UPDATE users SET active=0, session_version=session_version+1 WHERE email=?')->execute([$email]);echo "Account disabled.\n";exit;}
